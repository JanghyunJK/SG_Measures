...
class AddRedirectingBipv < OpenStudio::Measure::ModelMeasure
  ...
  def arguments(model)
    ...
    dot_coverage.setDefaultValue(0.50)
    args << dot_coverage
  ...
  def run(model, runner, user_arguments)
    ...
    candidate_windows.each do |surface|
      ...
      runner.registerInfo("window normal: #{surface.outwardNormal}") if debug_mode
