...
class AddIntervalScheduleFromFile < OpenStudio::Ruleset::ModelUserScript
  ...
  def arguments(model)
    ...
    schedule_name.setDefaultValue('testschedule')
    args << schedule_name
    ...
    file_path.setDefaultValue("C:/Users/JKIM4/Box Sync/Project Files/5 NEXT_SwitchGlaze/SimulationResults/PCE_Schedule_HOT_10.0_SUB SURFACE 1.csv")
    args << file_path
  ...
  def run(model, runner, user_arguments)
    ...
    ###############################################################################
    # assigning schedule object to simple pv object
    ###############################################################################
    generatorpvs = model.getGeneratorPhotovoltaics
    generatorpvs.each do |generatorpv|
      runner.registerInfo("################################################")
      surfacename = generatorpv.surface.get.name.to_s.split(" - ")[1]
      simplepv = generatorpv.photovoltaicPerformance.to_PhotovoltaicPerformanceSimple.get
      
      runner.registerInfo("generator pv name = #{generatorpv.name}")
      runner.registerInfo("surface name = #{surfacename}")
      runner.registerInfo("photovoltaic performance name = #{generatorpv.photovoltaicPerformance.name}")
      
      sub_surfaces = model.getSubSurfaces
      sub_surfaces.each do |sub_surface|
        
        if (sub_surface.outsideBoundaryCondition == 'Outdoors') && (sub_surface.subSurfaceType == 'FixedWindow') && (sub_surface.name.to_s == surfacename.to_s)
          runner.registerInfo("subsurface name = #{sub_surface.name}")
          vertices = sub_surface.vertices
          runner.registerInfo("vertices = #{vertices}")
    ...
    runner.registerInfo("################################################")
      ...
      simplepv.setEfficiencySchedule(schedule) #this automatically sets Conversion Efficiency Input Mode as "scheduled"      
    end
    
   
    #TODO:
    #how to store vertice info when saving to csv
    #how to read both surface T and vertice from csv
    #how to apply multiple schedules to BIPV surface
